...
Resolve `{workspace}` (see CONVENTIONS), then follow [CONVENTIONS.md](../jira-acceptance-criteria-check/CONVENTIONS.md) § Historical artifact read lookup.
...
Prefer a **structured choice prompt** when the host supports multiple-choice UI. **Wait** for the user's choice before continuing.
...
Load `meta.storyPointsField` when present; otherwise use the default from `scripts/lib/constants.mjs` ([CONVENTIONS.md](../jira-acceptance-criteria-check/CONVENTIONS.md)).
