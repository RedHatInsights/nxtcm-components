/* Derived from @patternfly-labs/react-form-wizard InputSelect (Apache-2.0). */
import {
  Button,
  Divider,
  Label,
...
import type { Option, OptionType } from './RosaSelectTypes';
import type { NormalizedOptionGroup } from './RosaSelectTypes';
  ...
  useEffect(() => {
    ...
    const input =
      node && typeof node.querySelector === 'function'
        ...
        : node;
    if (input && typeof (input as HTMLInputElement).value !== 'undefined') {
...
type SelectListOptionsProps<T = unknown> = {
  value: string;
...
export function SelectListOptions<T>({
  value,
  options,
  optionGroups,
  isCreatable,
  ...
  isMultiSelect,
  isPending,
}: SelectListOptionsProps<T>) {
  const { noResults, createOption } = useWizardFooterStrings();
