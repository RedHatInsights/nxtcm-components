...
import { clusterValidationSchema, ValidationSchemaContext } from './validationSchemaFlat';
...
describe('clusterValidationSchema (flat / withContext)', () => {
  // -----------------------------------------------------------------------
  // Cluster name
  // -----------------------------------------------------------------------
  describe('name', () => {
  ...
  describe('min_replicas', () => {
    ...
    it('rejects value exceeding 500', async () => {
      const error = await validate(
  ...
  describe('network_machine_cidr', () => {
    ...
    it('rejects mask larger than multi-AZ max (/24) when multi_az', async () => {
      const error = await validate(
        buildContext(),
        buildFormData({ network_machine_cidr: '10.0.0.0/25', multi_az: 'true' }),
        field
      );
  ...
  describe('network_pod_cidr', () => {
    ...
    it('rejects when pod CIDR does not allow enough nodes', async () => {
      const error = await validate(
        buildContext(),
        buildFormData({ network_pod_cidr: '10.128.0.0/21', network_host_prefix: '/21' }),
        field
      );
    ...
    it('collects multiple errors with abortEarly: false', async () => {
      ...
      try {
        await clusterValidationSchema.validate(
          buildFormData({ name: '', cluster_version: '', region: '' }),
